...
class Sketch(CanvasBase):
    ...
    * buff_last: Buff. Last frame buffer
    ...
    def drawLine(self, buff, p1, p2, doSmooth=True, doAA=False, doAAlevel=4):
        ...
        # Define err to help with deciding whether to move to x or y.
        err = dx - dy

        while True:
            if doSmooth:
                # Calculate the color of current point
                total_dist = math.sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2)
                current_dist = math.sqrt((p1.coords[0] - x1) ** 2 + (p1.coords[1] - y1) ** 2)
                t = 0
                if total_dist != 0:
                ...
                g = max(0, min(1, g))
                b = max(0, min(1, b))
                point = Point((x1, y1), ColorType(r, g, b))
            else:
                point = Point((x1, y1), c1)

            self.drawPoint(buff, point)

            # When arriving end, stop loop.
            if x1 == x2 and y1 == y2:
                break

            if err > 0:  # err is bigger than 0，so move to x
                x1 += sx
                err -= dy
            else:  # err is equal to or less than 0，so move to y
                y1 += sy
                err += dx
    ...
    def drawTriangle(self, buff, p1, p2, p3, doSmooth=True, doAA=False, doAAlevel=4, doTexture=False):
        ...
        # Make sure the y-coordinate of p1 <= the y-coordinate of p2 <= the y-coordinate of p3
        if p1.coords[1] > p2.coords[1]:
            p1, p2 = p2, p1
        if p1.coords[1] > p3.coords[1]:
            p1, p3 = p3, p1
        if p2.coords[1] > p3.coords[1]:
            p2, p3 = p3, p2

        # Help us to calculate x and color
        def interpolate(y, p_start, p_end):
            if p_start.coords[1] == p_end.coords[1]:
                return p_start.coords[0], p_start.color
            t = (y - p_start.coords[1]) / (p_end.coords[1] - p_start.coords[1])
            x = p_start.coords[0] + t * (p_end.coords[0] - p_start.coords[0])
            r = p_start.color.r + t * (p_end.color.r - p_start.color.r)
            g = p_start.color.g + t * (p_end.color.g - p_start.color.g)
            b = p_start.color.b + t * (p_end.color.b - p_start.color.b)
            return int(x), ColorType(r, g, b)

        for y in range(p1.coords[1], p3.coords[1] + 1):
            if y < p2.coords[1]:
                x_start, color_start = interpolate(y, p1, p2)
            else:
                x_start, color_start = interpolate(y, p2, p3)

            x_end, color_end = interpolate(y, p1, p3)
            ...
            for x in range(x_start, x_end + 1):
                if doTexture:
                    ...
                    r = color_start.r + t * (color_end.r - color_start.r)
                    g = color_start.g + t * (color_end.g - color_start.g)
                    b = color_start.b + t * (color_end.b - color_start.b)
                    r = max(0, min(1, r))
                    g = max(0, min(1, g))
                    b = max(0, min(1, b))
